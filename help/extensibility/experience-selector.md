...
>
>Si vous souhaitez utiliser l’API GenStudio pour gérer les expériences, consultez la [documentation de l’API GenStudio](https://developer.adobe.com/genstudio-api/).
...
Pour incorporer ce MFE dans Salesforce Lightning avec LWC et des modèles d’e-mail facultatifs, voir [Sélecteur d’expérience MFE dans Salesforce](experience-selector-salesforce.md).
...
| `imsOrg` | chaîne | Oui | Identifiant IMS de l’organisation |
...
Lorsqu’une [!DNL Experience] est sélectionnée dans le MFE et que vous cliquez sur le bouton **[!UICONTROL Utiliser]**, une fonction de rappel doit être appelée pour transmettre les détails de la [!DNL Experience] sélectionnée.

Le rappel fournit une chaîne HTML représentant l’[!DNL Experience]. Si l’expérience comprend des ressources (des images ou des vidéos, par exemple), les éléments HTML, tels que `<img>` et `<video>`, incluent alors des attributs `src` qui contiennent des URL présignées pour ces ressources. Ces URL présignées ont un délai d’expiration de 30 minutes et l’[!DNL Experience] doit être consommée par l’application cliente pendant cette période.

### Schéma de sortie

Le rappel `onSelectionConfirmed` reçoit un tableau d’objets [!DNL Experience] avec la structure suivante :

| Propriété | Type | Description |
|----------|------|-------------|
| `id` | `string` | Identifiant unique du [!DNL Experience] (format URN). |
| `content` | `string` | Chaîne HTML codée en Base64 représentant l’[!DNL Experience]. |
| `experienceFields` | `object` | Paires clé-valeur de champs modifiables dans le [!DNL Experience]. Chaque champ de `experienceFields` contient : <ul><li>`fieldName` : nom/identifiant du champ.</li><li>`fieldValue` : contenu/valeur réel du champ.</li></ul> |
| `metadata` | `object` | Métadonnées sur l’expérience (par exemple, `channel`, `externalTemplateMetadata`, `externalAssetMetadata`, etc.) |
| `aspectVariants` | `array` | Tableau de variantes des proportions pour le [!DNL Experience]. Chaque variante contient : <ul><li>`aspectKey` : identifiant unique de la variante.</li><li>`aspectMetadata` : métadonnées, y compris `channel` et `aspectRatio`.</li><li>`content` : chaîne HTML codée en Base64 pour ce format spécifique.</li></ul> |

### Exemple JSON

Vous trouverez ci-dessous un exemple de bloc JSON affichant les schémas.
