...
- Règles de curseur : l’automatisation et le guidage personnalisés se trouvent dans `.cursor/rules/*.mdc`. Exemples : `.cursor/rules/docs-lint.mdc` (processus de lint), `.cursor/rules/generate-release-notes.mdc` (pointe vers les compétences des notes de mise à jour lors de la modification de `help/user-guide/release-notes.md`). Suivez ces instructions pour les tâches automatisées.
- Compétences de l’agent : les workflows de tâche sont actifs sous `.cursor/skills/<name>/SKILL.md`. Exemple : `.cursor/skills/generate-release-notes/SKILL.md` (procédure des notes de mise à jour complètes de GenStudio ; à utiliser avec `examples.md` et `reference.md` dans le même dossier).
- Noms de fichiers et frontend :
   - Notes de mise à jour : conservation du frontMATTER Experience League existant sur `help/user-guide/release-notes.md` (voir `.cursor/skills/generate-release-notes/reference.md#frontmatter`) ; la règle de fine `.cursor/rules/generate-release-notes.mdc` se joint automatiquement lorsque ce fichier est modifié.
...
- N’ajoutez jamais d’ID Jira, de liens internes ou de références d’entreprise uniquement aux documents publics. Voir `.cursor/skills/generate-release-notes/SKILL.md` (section **Contenu interdit**).
